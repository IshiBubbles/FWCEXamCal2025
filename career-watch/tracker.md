...
> ⚠️ **Tetra Tech — location problem found 20 August 2026, check before spending effort.** It is a genuine live vacancy rather than a register-interest form, but five independent searches returned **Edinburgh** for this exact job title ("join their Renewable Energy Infrastructure team in Edinburgh") — roughly 470 miles away, not the UK-wide posting previously assumed. That is snippet-based, so **confirm on the live page first**. The deadline is **rolling** (apply until filled), so there is no date to miss but equally no protection. Tetra Tech does have a **Southampton office (SO18 2RZ)**, which makes "can this role be hosted from Southampton?" a strong, informed enquiry rather than simply dropping it. Full detail in [research/tier1-civil.md](research/tier1-civil.md).

| Employer | Role | Location | Link | Status |
|---|---|---|---|---|
| **Tetra Tech** | Degree Apprentice — Civil Engineer, Renewable Energy Infrastructure | ⚠️ **Edinburgh reported (~470mi)** — verify | [GradCracker](https://www.gradcracker.com/hub/1108/tetra-tech/degree-apprenticeship/81851/degree-apprentice-civil-engineer-renewable-energy-infrastructure) | 🟠 **Live vacancy, rolling deadline — but check the location first** |
...
**Action — Tetra Tech, but check before writing:** open the live vacancy and settle two things before any effort goes into a CV or personal statement — **where the role actually is**, and whether it can be hosted from Tetra Tech's Southampton office. The deadline is rolling rather than fixed, so there is no clock forcing a same-day application, which makes the location question worth answering first. If it is genuinely Edinburgh-only, redirect the effort. The other four are register-interest forms — quick to do, worth doing this week, no clock running.
...
|---|---|---|
| **National Grid** | 🔴 **Opens September 2026 — most time-critical item.** Needs A-levels. No visa sponsorship. Closes early if full. | Eakring/Warwick/Wokingham |
...
**Worth adding to the follows:** Babcock International · GKN Aerospace · WSP · L3Harris

---
Last full refresh: **17 August 2026** — treated as a first run, all entries re-verified. Next review should be **1 September 2026** for the National Grid opening.
